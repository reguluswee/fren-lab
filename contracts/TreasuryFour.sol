 ...
 */
contract TreasuryFour is Ownable, ReentrancyGuard, IBurnRedeemable, IERC165 {
    ...
    /************** two new methods for treasury support 20 Tokens *****************/
    event AddToken(address indexed tokenAddr);

    function relayTokenApprove(address token) external onlyOwner {
        Ut20(token).approve(address(this), ~uint256(0));
        emit AddToken(token);
    }

    function withdrawToken(address token, address to, uint256 amount) external onlyOwner {
        if(Ut20(token).balanceOf(address(this)) > 0) {
            amount = amount == 0 ? Ut20(token).balanceOf(address(this)) : amount;
