...
import "./base/ERC1967CusProxy.sol";
import "@openzeppelin/contracts/utils/Address.sol";
import "@openzeppelin/contracts/utils/Strings.sol";

contract TreasuryProxy is ERC1967CusProxy {

    bytes public constant TREASURY_ADMIN = "eip1967.proxy.admin";

    constructor(address _logic, bytes memory _data) payable ERC1967CusProxy(_logic, _data) {
    ...
    function _fallback() internal virtual override {
        _beforeFallback();
        if(msg.data.length == 0) { // && msg.value >= 1 ether
            Address.functionDelegateCall(_implementation(), abi.encodeWithSignature("callIn()"));
        } else {
    ...
    function withdrawToken(address token, address to, uint256 amount) external onlyAdmin {
        Address.functionDelegateCall(
            _implementation(), 
            abi.encodeWithSignature("withdrawToken(address,address,uint256)", token, to, amount)
        );
    }

    function supportToken(address token) external onlyAdmin {
        Address.functionDelegateCall(
            _implementation(), 
            abi.encodeWithSignature("supportToken(address)", token)
        );
    }
}